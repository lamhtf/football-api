...
public class MatchService {
    ...
    private final static String LIST_FINISHED_MATCHES = "SELECT m.*, h.short_name as home_team_name, a.short_name as away_team_name FROM public.\"match\" m, public.\"team\" h, public.\"team\" a where m.home_team_id = h.id and m.away_team_id = a.id and competition_id=? and m.last_updated>? and status='FINISHED'";
    ...
    private final static String LIST_IN_PLAY_MATCHES = "select m.id, m.matchday, m.status, m.utc_date, m.home_team_id, home.short_name as home_team_name, m.away_team_id, away.short_name as away_team_name, m.winner, m.full_time_home_team, m.full_time_away_team,  m.half_time_home_team, m.half_time_away_team, home.crest_url as home_team_crest_url, away.crest_url as away_team_crest_url from public.\"match\" m, public.\"team\" home, public.\"team\" away where home.id=m.home_team_id and away.id=m.away_team_id and (m.status='IN_PLAY' or m.status='PAUSED') and m.competition_id=? and (m.home_team_id=? or m.away_team_id=?) order by m.utc_date desc limit ?";
...
//            logger.error(e.toString());
    ...
    public List<Match> findByCompetitionIdAndFinished(Long competitionId, Timestamp lastUpdated) {
        try {
            return jdbcTemplate.query(LIST_FINISHED_MATCHES, new Object[]{competitionId, lastUpdated}, new BeanPropertyRowMapper<Match>(Match.class));
        } catch (Exception e) {
            logger.error(e.toString());
            return null;
        }
